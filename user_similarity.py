...
def cosine_similarity(collection, db_image_data, input_user_data, k, model):
    cosine_sim_output = []
    for record in db_image_data:
        ...
        for i in range(1, len(input_user_data[1][1:]) + 1):
            x = input_user_data[1][i]
            y = record[1][i]
            sumxx += x * x
            sumyy += y * y
            sumxy += x * y
...
def main():
    ...
    collection_name = 'user_text_descriptors'
    try:
    ...
    user_text_desc_path = os.getcwd() + '\dataset\desctxt\devset_textTermsPerUser.txt'
    with open(user_text_desc_path, 'r', encoding='UTF-8') as fileobject:
    ...
    # Taking user Input
    user_input = input('Please enter the user id, model and k : ')
    input_user_id = user_input.split(' ')[0]
    model = user_input.split(' ')[1]
    k = int(user_input.split(' ')[2])

    # Creating Input User Vector
    input_user_data_terms = []
    input_user_data_freq = []
    for record in collection.find({'_id': input_user_id}).sort('DATA.TERM', pm.ASCENDING):
        input_user_data_terms.append(record['_id'])
        input_user_data_freq.append(record['_id'])
        for data in sorted(record['DATA'], key=lambda z: z['TERM']):
            input_user_data_terms.append(data['TERM'])
            input_user_data_freq.append(data[model])
    input_user_data = [input_user_data_terms, input_user_data_freq]

    # Creating a Vector of all other database users
    db_image_data = []
    for records in collection.find({'DATA.TERM': {'$in': input_user_data_terms[1:]}}):
        db_image_data_terms = []
        db_image_data_tf = []
        x = []
        db_image_data_terms.append(records['_id'])
        db_image_data_tf.append(records['_id'])
        for i in range(1, len(input_user_data_terms)):
            term_found = False
            for data in sorted(records['DATA'], key=lambda z: z['TERM']):
                if data['TERM'] == input_user_data_terms[i]:
                    db_image_data_terms.append(data['TERM'])
                    db_image_data_tf.append(data[model])
                    term_found = True
                    break
            if term_found is False:
                db_image_data_terms.append(0)
                db_image_data_tf.append(0)
        while len(db_image_data_terms) < len(input_user_data_terms):
            db_image_data_terms.append(0)
            db_image_data_tf.append(0)
        x.append(db_image_data_terms)
        x.append(db_image_data_tf)
        db_image_data.append(x)

    # Passing Input User Vector and Database User Vectors for Cosine Similarity Calculations
    cosine_similarity(collection, db_image_data, input_user_data, k, model)
    end_time = time.time()
